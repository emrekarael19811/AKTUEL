-- ============================================================
-- AKTUEL - Supabase Schema Migration
-- Supabase Dashboard > SQL Editor'a yapistirin ve Run'a basin.
-- ============================================================

-- pgvector extension (semantic search icin)
CREATE EXTENSION IF NOT EXISTS vector;

-- timescaledb extension (fiyat tarihcesi icin - Supabase'de desteklenir)
-- CREATE EXTENSION IF NOT EXISTS timescaledb; -- Pro plan gerektirir
...
CREATE INDEX IF NOT EXISTS idx_catalogs_market_valid ON catalogs(market_id, valid_from, valid_until);
...
-- ============================================================
-- 4. CANONICAL PRODUCTS (Tekillestirilmis urunler)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_canonical_name ON canonical_products(normalized_name);
CREATE INDEX IF NOT EXISTS idx_canonical_brand ON canonical_products(brand);
CREATE INDEX IF NOT EXISTS idx_canonical_barcode ON canonical_products(barcode) WHERE barcode IS NOT NULL;

-- IVFFlat index - cosine similarity ile yakin urun bulma
CREATE INDEX IF NOT EXISTS idx_canonical_embedding ON canonical_products
...
-- ============================================================
-- 5. RAW PRODUCTS (OCR'dan gelen ham urunler)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_raw_products_catalog ON raw_products(catalog_id);
CREATE INDEX IF NOT EXISTS idx_raw_products_canonical ON raw_products(canonical_product_id) WHERE canonical_product_id IS NOT NULL;

-- ============================================================
-- 6. PRODUCT PRICES (Fiyat zaman serisi)
-- ============================================================
CREATE TABLE IF NOT EXISTS product_prices (
    ...
    discount_pct          FLOAT,
    -- Arama siralamasinin temeli: 100g veya 100ml baz fiyati
    unit_price_tl         FLOAT,
    valid_from            DATE    NOT NULL,
    valid_until           DATE    NOT NULL,
    created_at            TIMESTAMPTZ DEFAULT NOW()
...
CREATE INDEX IF NOT EXISTS idx_prices_valid_market   ON product_prices(valid_from, market_id);
CREATE INDEX IF NOT EXISTS idx_prices_canonical_valid ON product_prices(canonical_product_id, valid_from);
CREATE INDEX IF NOT EXISTS idx_prices_unit_price      ON product_prices(unit_price_tl ASC NULLS LAST);

-- ============================================================
-- 7. USERS (Supabase Auth ile entegre)
-- ============================================================
...
-- Supabase Auth ile senkronizasyon: yeni kullanici kaydi otomatik ekle
...
-- ============================================================
-- 8. WATCHLIST (Kullanici takip listesi)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_watchlist_user ON watchlist(user_id, is_active);
...
-- ============================================================
-- 10. ROW LEVEL SECURITY (RLS) - KVKK uyumu
-- ============================================================

-- Kullanicilar sadece kendi verilerini gorsun
...
-- ============================================================
-- 11. YARDIMCI VIEW: Aktif kampanyalar
-- ============================================================
